...
BEGIN_MESSAGE_MAP(CSubclassView, CFormView)
	...
	ON_BN_CLICKED(IDC_CHECK1, &CSubclassView::OnBnClickedCheck1)
END_MESSAGE_MAP()
...
void CSubclassView::OnBnClickedCheck1()
{

	BOOL bCheck = m_check1.GetCheck();
	if (bCheck)
	{
		if (::IsWindow(m_edit3.GetSafeHwnd()) != FALSE)
			m_edit3.UnsubclassWindow();
		m_edit2.SubclassDlgItem(IDC_EDIT2, this);
		m_edit2.SetWindowText(_T(""));
	}
	else {
		if (::IsWindow(m_edit2.GetSafeHwnd()) != FALSE)
			m_edit2.UnsubclassWindow();

		m_edit3.SubclassDlgItem(IDC_EDIT2, this);
...
//void CSubclassView::OnNcXButtonDblClk(short nFlags, UINT nButton, CPoint point)
//{
	// 이 기능을 사용하려면 Windows 2000 이상이 필요합니다.
	// _WIN32_WINNT 및 WINVER 기호는 0x0500보다 크거나 같아야 합니다.
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.

//	CFormView::OnNcXButtonDblClk(nFlags, nButton, point);
//}


//void CSubclassView::OnNcMButtonDblClk(UINT nHitTest, CPoint point)
//{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.

//	CFormView::OnNcMButtonDblClk(nHitTest, point);
//}


//void CSubclassView::OnXButtonDblClk(UINT nFlags, UINT nButton, CPoint point)
//{
	// 이 기능을 사용하려면 Windows 2000 이상이 필요합니다.
	// _WIN32_WINNT 및 WINVER 기호는 0x0500보다 크거나 같아야 합니다.
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.

//	CFormView::OnXButtonDblClk(nFlags, nButton, point);
//}


//void CSubclassView::OnRButtonDblClk(UINT nFlags, CPoint point)
//{
	// TODO: 여기에 메시지 처리기 코드를 추가 및/또는 기본값을 호출합니다.
